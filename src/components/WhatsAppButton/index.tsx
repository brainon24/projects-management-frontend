...
export const WhatsAppButton = ({ 
  phoneNumber = "+573182658901",
  message = "Hola, me interesa conocer más sobre sus servicios",
  floatingMessage = "¿Necesitas ayuda? 👋 ¡Escríbenos!"
}: WhatsAppButtonProps) => {
  ...
  useEffect(() => {
    ...
    // Solo mostrar el mensaje en desktop (no en móvil)
    let showMessageTimer: number;
    let hideMessageTimer: number;
    ...
    return () => {
      ...
      if (hideMessageTimer) clearTimeout(hideMessageTimer);
  ...
  useEffect(() => {
    ...
    // Solo ocultar si no hay hover en el container
    if (!isHoveringContainer && !isMobile) {
  ...
  return (
    ...
    >
      {(showMessage || isHoveringContainer) && !isMobile && (
